 ...
 *          several 16-bit timers (TIM3, TIM4, etc).  The left encoder
 *          uses TIM2 (32-bit) so its counter never wraps during any
 *          realistic maze run — no overflow handling is needed at all.
 *
 *          The right encoder uses TIM3 (16-bit) because TIM2 is already
 ...
 *          (encoders_tim3_ovf_irq) tracks every wrap to extend the
 *          effective range to 32 bits.
 *
 *          OVERFLOW DIRECTION DETECTION
 *          ─────────────────────────────
 *          When TIM3 wraps, its update interrupt fires.  At that moment
 *          TIM3->CNT has already rolled over.  We determine direction by
 ...
 *          ────────────────────────────
 *          right_count = (overflow × 65536) + TIM3->CNT
 ...
 *          ──────────────────────────
 *          Between reading TIM3->CNT and reading s_right_overflow, the
 *          TIM3 IRQ could fire and increment the overflow counter while
...
#include "pins.h"
#include "config.h"
#include "main.h"     /* htim2, htim3 externs */

/* =========================================================================
 * PRIVATE STATE
 * ======================================================================= */

/** TIM3 16-bit overflow/underflow extension counter.
...
/** Last TIM3 CNT value observed in the overflow ISR.
...
/**
 * @brief  Read TIM3 CNT combined with overflow counter into a 32-bit value.
 *
 * @details Implements the double-read anti-race technique:
 *          1. Read TIM3->CNT (first read).
 *          2. Read s_right_overflow.
 *          3. Read TIM3->CNT again (second read).
...
{
    /* TIM2 and TIM3 must already be configured in encoder mode by main.c */
...
{
    /* Disable TIM3 update interrupt during the reset to prevent the
...
/**
 * @brief  TIM3 overflow/underflow ISR hook.
 *
 * @details Called from HAL_TIM_PeriodElapsedCallback in main.c when
 *          htim->Instance == TIM3.  Runs inside a hardware interrupt.
 *
 *          DIRECTION DETERMINATION
 *          ────────────────────────
 *          After a wrap the new TIM3->CNT value tells us which way:
 *
