 ...
 *          Right wheel : TIM3 (16-bit, PB4/PB5) — overflow tracked via IRQ.
 *
 ...
 *          TIM3->CNT with a volatile overflow counter that is updated
 *          in the TIM3 IRQ.  Read it with interrupts disabled if you
...
typedef struct
{
    ...
    uint32_t overflow_count;   /**< TIM3 16-bit overflow event counter    */
} EncoderState_t;
 ...
 *
 * @details Writes 0 to TIM2->CNT and TIM3->CNT, clears the overflow
 ...
 *
 * @details Combines TIM3->CNT (16-bit) with an overflow counter that is
 *          incremented/decremented in encoders_tim3_ovf_irq() to provide
 *          a 32-bit range.  Applies RIGHT_ENC_POL.
 *
 * @warning Not atomic — the 16-bit CNT read and the overflow counter read
 *          can be split by the TIM3 IRQ.  This is handled internally by
...
/**
 * @brief  TIM3 overflow/underflow ISR hook — call from stm32f4xx_it.c.
 *
 * @details TIM3 is a 16-bit counter that overflows at 65535 and underflows
 ...
 *
 *          MUST be called from TIM3_IRQHandler via
 *          HAL_TIM_PeriodElapsedCallback when htim->Instance == TIM3.
 *          Must execute in under 1 µs — it is in the hot ISR path.
 *
 *          How overflow direction is determined:
 *            - If TIM3->CNT is near 0     after the event → forward overflow
 *              (count crossed 65535 → 0, wheel moving forward)
 *            - If TIM3->CNT is near 65535 after the event → reverse underflow
