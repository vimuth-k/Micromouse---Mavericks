 ...
 *   ~20 µs for the 2-byte blocking I2C read. It must never log
 *   (LOG_* is not ISR-safe — see logger.h) and never block longer than
 *   the I2C timeout. All other imu_* functions are safe to call only
 *   from main-loop context (init, calibration, and status queries).
 *
 * @author  VDawn
 * @date    2026
 */
