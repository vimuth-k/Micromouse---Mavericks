 ...
 * APB1 = 50 MHz (TIM2/TIM3/TIM5 × 2 = 100 MHz timer clock).
...
/** APB1 timer clock (TIM2, TIM3, TIM5): APB1 × 2 = 100 MHz */
 ...
 *
 * @note   Currently 0U because no debug transport is available without
 *         sacrificing a pin. UART1 has been removed entirely (PA9 is
 *         TIM1_CH2, motor PWM — a hard conflict). USB CDC(PA11/PA12)=
 *         L_ANGLE_EM+DIP1, UART2(PA2)=IR receiver, UART6(PC6)=doesn't
 *         exist on this package, SWO(PB3)=LF_EM. OLED is the only live
 *         status output for now — see oled_show_message()/
 *         oled_show_error() call sites. Flip back to 1U (and re-add a
 *         real transport in logger.c) if a debug pin is ever freed up
 *         (temporary DIP borrow, or giving up an IR emitter).
 */
...
/* =========================================================================
 * SECTION 18 — CALIBRATION UX TIMING
 * =========================================================================
...
/**
 * @brief  Maximum time to wait for the operator to reposition the robot
 *         and press the confirm button between each IR calibration step
 *         (ms).
 * @details Step 1 (ir_cal_ambient()) needs the robot in open space,
 *          Step 2 (ir_cal_wall()) needs it in the start cell against
 *          three walls at 90 mm. calibration.c waits on
 *          buttons_wait_for_press() rather than a fixed delay, so the
 *          operator can take as long as they need — this is just an
 *          upper bound so calibration.c can't hang forever if the
 *          button is unpressed (or faulty); it logs a warning and
 *          proceeds anyway if this elapses with no press.
 */
#define CAL_IR_CONFIRM_TIMEOUT_MS  30000U

/**
 * @brief  Extra settle time after the confirm press, immediately before
 *         each capture (ms).
 * @details The IR DMA scan (ir.c) runs continuously in the background,
 *          so this is a small margin for the operator's hand to be
 *          clear of the robot and away from the button after pressing
 *          it, and for readings to stabilise — not a hardware
 *          requirement.
 */
#define CAL_IR_SETTLE_MS         200U

/* =========================================================================
 * SECTION 19 — STARTUP SELF-TEST PARAMETERS
 * =========================================================================
...
/* =========================================================================
 * SECTION 21 — WALL FOLLOWER PARAMETERS
 * =========================================================================
 * Controls Control/wall_follow.c (DIP Mode 5) — the left-hand-rule
 * fallback maze solver.
 * ======================================================================= */

/**
 * @brief  Maximum cells wall_follow_run() will traverse before giving
 *         up and reporting failure.
 * @details Safety net, not a normal stopping condition: left-hand-rule
 *          is only guaranteed to reach the goal in a simply-connected
 *          maze (no wall loops fully disconnected from the boundary).
 *          500 comfortably covers a full MAZE_SIZE×MAZE_SIZE (256-cell)
 *          traversal with revisits, without looping forever on a
 *          pathological layout.
 */
#define WALL_FOLLOW_MAX_CELLS    500U

/* =========================================================================
 * SECTION 22 — EXPLORER PARAMETERS
 * =========================================================================
 * Controls Navigation/explorer.c (DIP Mode 6) — the flood-fill search
 * and return-to-start algorithm.
 * ======================================================================= */

/**
 * @brief  Maximum cells explorer_search_to_goal() or
 *         explorer_return_to_start() will traverse before giving up.
 * @details Unlike WALL_FOLLOW_MAX_CELLS, flood-fill is a complete
 *          search — a correctly-connected maze can't produce an
 *          unbounded loop the way left-hand-rule can. This is a bug/
 *          sanity safety net (e.g. a floodfill_best_direction()
 *          returning DIR_NONE unexpectedly) rather than an expected
 *          stopping condition; safety.c's own run-timeout and
 *          max-distance watchdogs are the primary real-world guard.
 */
#define EXPLORER_MAX_CELLS       500U

/* =========================================================================
 * SECTION 23 — DIAGNOSTICS DISPLAY TIMING
 * =========================================================================
 * Controls System/diagnostics.c (DIP Modes 0, 11, 12, 13) — the live
 * OLED/UART view modes (sensor monitor, gyro debug, battery check).
 * ======================================================================= */

/**
 * @brief  OLED/UART refresh interval for diagnostics.c's live-view
 *         loops (ms).
 * @details 100 ms (10 Hz) is fast enough to feel live to a human
 *          observer without flooding the UART log or redrawing the
 *          OLED faster than it's useful to read.
 */
#define DIAG_REFRESH_MS          100U

/* =========================================================================
 * SECTION 24 — MODE TEST PARAMETERS
 * =========================================================================
 * Controls System/modes.c — parameters intrinsic to a specific DIP
 * mode's own test definition (MOTOR_TEST reuses
 * STARTUP_TEST_PWM/STARTUP_TEST_MOTOR_MS from Section 20 rather than
 * duplicating them here, since it's the same "brief low-power spin,
 * check encoder" pattern applied to one motor at a time).
 * ======================================================================= */

/**
 * @brief  Cells driven by MODE_STRAIGHT_TEST (DIP Mode 3).
 * @details Matches the DIP mode table's own description ("Drive 5
 *          cells, check drift") — this is part of the mode's
 *          definition, not an independently tunable parameter.
 */
#define STRAIGHT_TEST_CELLS      5U

/* =========================================================================
 * SECTION 25 — DERIVED CONSTANTS  (do not edit — computed from above)
 * =========================================================================
...
/* =========================================================================
 * SECTION 26 — COMPILE-TIME ASSERTIONS
 * =========================================================================
