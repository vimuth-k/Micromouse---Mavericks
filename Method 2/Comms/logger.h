 ...
 *   USB CDC (see logger.c's file banner — currently stubbed out, no
 *   transport is free on this board's pin map yet).
 *
 ...
 *   and a blocking transmit call (once a real transport exists — see
 *   logger.c's file banner), neither of which is interrupt-safe or
 *   fast enough for a 1 kHz deadline. Every existing call site is in
 *   main-loop context (main.c, scheduler.c, maze.c, path_optimizer.c)
 *   — keep it that way.
 *
 * @author  VDawn
 * @date    2026
...
/**
 * @brief  Bring the logger online.
 *
 * @details Resets the active level to LOG_LEVEL_DEFAULT (from config.h)
 *          and marks the module initialised. Call once from main.c
 *          before any other module that might log during its own init.
 *
 *          There is currently no live transport (see logger.c's file
 *          banner) — USART1 is blocked by a hard pin conflict (TX=PA9
 *          is also TIM1_CH2, motor PWM), and USB CDC is stubbed out
 *          pending a pin becoming free (the Black Pill's USB connector
 *          is hardwired to PA11/PA12, currently L_ANGLE_EM and DIP1).
 *          Every LOG_* call still compiles and runs (with LOG_ENABLED=1)
 *          but log_transmit() has nowhere to send bytes until one of
 *          those is resolved. With LOG_ENABLED=0 (the current setting
 *          in config.h), this is moot — the calls compile to nothing.
 */
 ...
 *          blocks until its own bytes are on the wire (once a real
 *          transport is wired in — see logger.c's file banner), so
 *          there is never anything left pending when logger_log()/logger_raw() return. logger_flush() is a
