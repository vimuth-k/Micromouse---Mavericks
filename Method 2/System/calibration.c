...
MmResult_t calibration_run_ir(void)
{
    ...
    oled_show_message("IR CAL 1/2", "PRESS: OPEN");
    if (!buttons_wait_for_press(CAL_IR_CONFIRM_TIMEOUT_MS))
    {
        LOG_WARN("IR calibration: step 1 confirm timed out — proceeding anyway");
    }
    HAL_Delay(CAL_IR_SETTLE_MS);
    ir_cal_ambient();

    LOG_INFO("IR calibration: step 2/2 — place robot in start cell");
    oled_show_message("IR CAL 2/2", "PRESS: START");
    if (!buttons_wait_for_press(CAL_IR_CONFIRM_TIMEOUT_MS))
    {
        LOG_WARN("IR calibration: step 2 confirm timed out — proceeding anyway");
    }
    HAL_Delay(CAL_IR_SETTLE_MS);
    ir_cal_wall();
