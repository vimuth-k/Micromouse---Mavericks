 ...
 *
 * @details logger.c transmits every LOG_* call synchronously — nothing
 *          is buffered, so logger_flush() is currently a documented
 *          no-op (see logger.h). This slot is kept registered as the
 *          extension point for if/when logger.c moves to a buffered or
 *          DMA-driven transmit, at which point this call starts doing
