                               ...
                               float          run_speed)
{
    ...
    if (s_path.total_cells != dir_count)
    {
        ...
        LOG_ERROR("path_optimizer: cell count mismatch (%u vs %u)",
                  s_path.total_cells, dir_count);
        return MM_ERR_OVERFLOW;
    }
...
{
    const OptPath_t *p = (path != NULL) ? path : &s_path;

    LOG_INFO("Optimised path: %u moves, %u cells, %u turns @ %.0f mm/s",
             p->count - 1U,   /* subtract DONE sentinel */
             p->total_cells,
             p->total_turns,
             (double)p->run_speed);
    ...
    {
        const PathMove_t *m = &p->moves[i];
        switch (m->type)
        {
            case MOVE_FORWARD:
                LOG_INFO("  [%2u] FORWARD  %u cells @ %.0f mm/s",
                         i, m->cells, (double)m->speed);
                break;
            case MOVE_TURN_RIGHT:
                LOG_INFO("  [%2u] TURN_RIGHT  90 deg", i);
                break;
            case MOVE_TURN_LEFT:
                LOG_INFO("  [%2u] TURN_LEFT   90 deg", i);
                break;
            case MOVE_TURN_180:
                LOG_INFO("  [%2u] TURN_180  180 deg", i);
                break;
            case MOVE_DONE:
                LOG_INFO("  [%2u] DONE", i);
                break;
            default:
                LOG_WARN("  [%2u] UNKNOWN move type %u", i, m->type);
                break;
        }
    }
}
