 ...
 *   function, and mirrors the same data to the UART logger, once per
...
/**
 * @brief  MODE_MONITOR — live IR sensor view on OLED + UART.
 * @details Logs the loaded IR calibration thresholds once at entry
 *          (ir_cal_get()), then refreshes oled_show_sensors() and a
 *          matching LOG_RAW UART line every DIAG_REFRESH_MS until the
 *          button is pressed.
 ...
 *         and flood-fill distances via the UART logger.
 * @details Runs floodfill_run(FLOOD_TO_GOAL) so the printed distances
 *          reflect the current map, then calls maze_print(). No OLED
 *          output (the map doesn't fit meaningfully on a 128×64
 *          display) and no live loop — prints once and returns.
 * @return MM_OK always.
 */
