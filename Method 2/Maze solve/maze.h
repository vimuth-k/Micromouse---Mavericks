...
/**
 * @brief  Print the 16×16 wall map as ASCII art via UART logger.
 *
 * @details Outputs a grid with '+' at corners, '-' for horizontal walls,
 *          '|' for vertical walls, and the flood-fill distance (0–255) in
 *          each cell.  The robot's current position is marked with '@'.
 *          Takes ~300 ms to print at 115200 baud — call from main loop only.
 *
