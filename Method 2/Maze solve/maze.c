...
MmResult_t maze_save_to_flash(void)
{
    ...
    (void)HAL_FLASH_Lock();
    LOG_INFO("Maze map saved to Flash (%u cells visited)", maze_visited_count());
    return MM_OK;
}
...
MmResult_t maze_load_from_flash(void)
{
    ...
    LOG_INFO("Maze map loaded from Flash (%u cells visited)", maze_visited_count());
    return MM_OK;
}
...
/**
 * @brief  Print the 16×16 wall map as ASCII art via logger.
 *
 * @details Output format (4×4 excerpt):
 *
 *   +---+---+---+---+
 *   |  0| 14|   |   |    ← flood distance in each cell, ' ' if visited
 *   +   +---+   +---+    ← '-' = horizontal wall, '+' = corner
 *   | 15| @ |   |   |    ← '@' = robot position
 *   +---+---+---+---+
 *
 *   '+' always printed at corners.
 *   '---' for horizontal wall, '   ' for open North/South.
 *   '|'   for vertical   wall, ' '   for open East/West.
 */
void maze_print(const uint8_t flood[MAZE_SIZE][MAZE_SIZE])
{
    char row_buf[MAZE_SIZE * 4 + 4];
    char cell_buf[8];
    ...
    for (uint8_t r = 0U; r < MAZE_SIZE; r++)
    {
        ...
        {
            row_buf[bi++] = '+';
            bool north = (maze.walls[r][c] & WALL_N) != 0U;
            row_buf[bi++] = north ? '-' : ' ';
            row_buf[bi++] = north ? '-' : ' ';
            row_buf[bi++] = north ? '-' : ' ';
        }
        ...
        for (uint8_t c = 0U; c < MAZE_SIZE; c++)
        {
            bool west = (maze.walls[r][c] & WALL_W) != 0U;
            row_buf[bi++] = west ? '|' : ' ';

            bool is_robot = (r == maze.robot_row) && (c == maze.robot_col);
            if (is_robot)
            {
                row_buf[bi++] = ' ';
                row_buf[bi++] = maze_dir_char(maze.robot_heading);
                row_buf[bi++] = ' ';
            }
            else if (flood != NULL && flood[r][c] < 255U)
            {
                (void)snprintf(cell_buf, sizeof(cell_buf), "%3u", flood[r][c]);
                row_buf[bi++] = cell_buf[0];
                row_buf[bi++] = cell_buf[1];
                row_buf[bi++] = cell_buf[2];
            }
            else
            {
                row_buf[bi++] = ' ';
                row_buf[bi++] = maze.visited[r][c] ? '.' : ' ';
                row_buf[bi++] = ' ';
            }
        }
        /* Rightmost wall */
        bool east_last = (maze.walls[r][MAZE_SIZE-1U] & WALL_E) != 0U;
        row_buf[bi++] = east_last ? '|' : ' ';
        row_buf[bi]   = '\0';
        LOG_RAW("%s\r\n", row_buf);
    }
    ...
    {
        row_buf[bi++] = '+';
        bool south = (maze.walls[MAZE_SIZE-1U][c] & WALL_S) != 0U;
        row_buf[bi++] = south ? '-' : ' ';
        row_buf[bi++] = south ? '-' : ' ';
        row_buf[bi++] = south ? '-' : ' ';
    }
