 ...
 *   main.c owns (htim1, htim2, htim4, htim5, hadc1, hdma_adc1, hi2c1,
...
extern TIM_HandleTypeDef  htim4;      /**< Right encoder — TIM4 PB6/PB7 (16-bit) */
 ...
 *
 * @details Disables motors, sounds the buzzer error pattern, logs the
 *          failure, shows it on the OLED, then blinks the onboard LED
 *          forever in a pattern that encodes @p err (N long blinks,
 *          N = error code value) so the failure is diagnosable without
 *          a serial terminal connected. Never returns.
 *
