 ...
 *                      below.
 *
 *   The OLED is not independently tested (there is no read-back path
 *   for it), but the test sequence itself uses the OLED for step-by-
 *   step progress and ends with an explicit ALL PASS / FAIL message,
 *   so a completely dead OLED is obvious to the operator even without
 *   a formal PASS/FAIL line for the display itself.
 *
 ...
 *          on the OLED, and ends with an explicit ALL PASS / FAIL message on
 *          the OLED.
 *
