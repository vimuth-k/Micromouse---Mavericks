 ...
 *          scheduler_tick() runs inside the spin (not just between
 *          blocking calls) so safety_check() — stall/battery/run-
 *          timeout/max-distance/yaw-error, on a 50 ms slot — and the
 *          rest of the scheduler table (logging, OLED, heartbeat) keep
 *          running for the full duration of a move, including a single
 *          multi-second move_forward() covering many cells. Without
 *          this, a stall could only ever be caught between moves, never
 *          during the long straight run where one is most likely.
 *
 * @author  VDawn
 * @date    2026
 */
...
#include "imu.h"
#include "utils.h"
#include "scheduler.h"  /* scheduler_tick() — serviced during blocking spins, see below */
...
void move_forward(uint8_t cells, float speed_mmps)
{
    ...
    /* Block until ISR sets s_motion_done. scheduler_tick() runs here so
     * safety_check() (stall/battery/timeout/distance/yaw — 50 ms slot)
     * and the rest of the scheduler table are still serviced during a
     * move, not just between blocking calls — see the scheduler.c /
     * motion.c integration note below. */
    while (!s_motion_done) { scheduler_tick(); }
